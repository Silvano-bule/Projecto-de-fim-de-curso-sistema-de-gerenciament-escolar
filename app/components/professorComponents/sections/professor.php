    <section id="nota" class="p-4 ml-64 ">
        <div class="grid grid-row-2 gap-8">
            <section class="container px-4 mx-auto">
                <div class="flex flex-col gap-4 mt-6">
                    <div class="flex flex-row justify-between items-center mt-8">
                        <div>
                            <h1 class="text-2xl font-bold text-white">Notas e Avaliações</h1>
                            <p>As disciplina dependem do curso do aluno</p>
                        </div>
                        <button class="flex flow-row items-center bg-blue-500 text-white rounded-md px-3 cursor-pointer" id="nota" onclick="document.getElementById('modal_nota').showModal()">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                            </svg>
                            <div tabindex="0" role="button" class="m-2">Nova Nota</div>
                        </button>
                    </div>
                            ...
                            <tbody>
                                ...
                                <?php if (!empty($registroGeralProfessor)): ?>
                                    <?php foreach ($registroGeralProfessor as $tur): ?>
                                        <tr>
                                            <td><?= $tur['nome_aluno'] ?></td>
                                            <td><?= $tur['nome_curso'] ?></td>
                                            <td><?= $tur['disciplina'] ?></td>
                                            <td><?= $tur['valor'] ?></td>
                                            <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">
                                                <div class="dropdown dropdown-hover">
                                                    ...
                                                    <ul class="dropdown-content menu bg-base-100 rounded-box w-52 p-2 shadow-sm z-50 absolute right-0">
                                                        <li>
                                                            <a>
